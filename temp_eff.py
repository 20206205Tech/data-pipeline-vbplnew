...
# # Tạo mã đơn giản để rollback tất cả các văn bản về bước 5
# import psycopg2
# from environs import Env
# from loguru import logger
...
# def rollback_all_documents():
#     conn = psycopg2.connect(DATABASE_URL)
#     try:
#         with conn.cursor() as cur:
#             # Lệnh UPDATE không có mệnh đề WHERE sẽ áp dụng lên TOÀN BỘ bảng
#             query = """
#                 UPDATE "public"."document_state"
#                 SET workflow_id = 5,
#                     start_time = NOW() - INTERVAL '1 minute',
...
#             logger.success(f"✅ Đã rollback thành công toàn bộ {cur.rowcount} văn bản!")
...
# if __name__ == "__main__":
#     rollback_all_documents()

# Tạo mã mới để rollback   các văn bản về bước 5 nhưng chỉ áp dụng cho những văn bản "Hết hiệu lực một phần"
...
def rollback_partial_expired_documents():
    conn = psycopg2.connect(DATABASE_URL)
    try:
        with conn.cursor() as cur:
            # Query đã được nâng cấp: Tự động JOIN với dim_eff_status để tìm đúng trạng thái
            # theo tên ('Hết hiệu lực một phần') mà không cần biết chính xác ID là gì.
            query = """
                UPDATE "public"."document_state"
                SET workflow_id = 5,
                    start_time = NOW() - INTERVAL '1 minute',
                ...
                WHERE item_id IN (
                    ...
                    WHERE s."name" = 'Hết hiệu lực một phần'
                );
            """
            cur.execute(query)
            conn.commit()
            ...
            if cur.rowcount > 0:
                logger.success(
                    f"✅ Đã rollback thành công {cur.rowcount} văn bản 'Hết hiệu lực một phần' về bước 5!"
                )
            else:
                logger.info(
                    "ℹ️ Không tìm thấy văn bản nào có trạng thái 'Hết hiệu lực một phần' để rollback."
                )
...
if __name__ == "__main__":
    rollback_partial_expired_documents()
